...
import webbrowser
import score_trad_ph3_4 as trad
...
def fLoc_mancatGUI(vLocS_archivo):
    ...
    #Description label
    iLabel_description = ttk.Label(iFrame_description, text = 'The following concepts where not found in the Wordbank.\nThe tool pre assigned them to a category and are presented here for your revision.\n')
    ...
    iLabel_concept = ttk.Label(iFrame_columns, text = 'Pre assigned category')
    ...
    iString_action = tk.StringVar(value = '1')
    #Radio Buttons
    iRadiob_reject = ttk.Radiobutton(iFrame_actions, text = 'Reject all assignements and leave all concepts in "No category"', variable = iString_action, value='2')
    iRadiob_manual = ttk.Radiobutton(iFrame_actions, text = 'Accept all assignements', variable = iString_action, value='1')
        ...
        #Checking for root concept in map
            ...
            for i in vExtD_wordbank.keys():
                for j in range(len(vLocM_conceptsid)): 
                    if vLocM_conceptsid[j][0] == vExtS_rootconcept.lower():
                        vLocM_conceptsid[j][0] = 'R'
                    elif vLocM_conceptsid[j][0] == i[2]:
                        vLocM_conceptsid[j][0] = i[1]
                    else:
                        for subcat in vGloD_categories[i[2]]:
                            if vLocM_conceptsid[j][0] == subcat:
                                vLocM_conceptsid[j][0] = i[0] #[0] for Category, [1] for Subcategory
                    if vLocM_conceptsid[j][1] == i[2]:  
            ...
            oLoc_outputfile.writerow(['Number of categories (NCAT):', vLocI_numcat])
            oLoc_outputfile.writerow(['Number of interlinks (NIL):', vLocI_interlinks])

            vLocF_catscore = 0
            if vLocI_numcat > 0:
                vLocF_catscore = (vLocI_numconcepts)*((vLocI_interlinks) / (vLocI_numcat))
...
iTk_main.geometry('750x200+250+50')
