...
def f_help():
    ...
    iButton_detailedhelp = ttk.Button(iToplevel_help, text='Detailed help', command = f_detailedhelp)
    iButton_detailedhelp.grid(column = 0, row = 1)

    iButton_closehelp = ttk.Button(iToplevel_help, text='Close help', command = lambda: iToplevel_help.destroy())
...
def f_run():
    ...
    #No output file
        ...
        showerror(title = 'Error message:', message = 'Please select the outputfile.\n')
        
    else:    
        #Traditional
        if (iString_method.get() == '1'): 
            score_trad.f_simpleprinttrad(vExtS_rootconcept,vExtL_inputfiles,vExtS_outputfile)  
            showinfo(title = 'Scoring calculation', message = 'Method: Categorical \n Your results are ready!')

        #Categorical
        else: 
            score_cat.f_simpleprintcat(vExtS_rootconcept,vExtL_inputfiles,vExtS_outputfile)
            showinfo(title = 'Scoring calculation', message = 'Method: Categorical \n Your results are ready!')
...
#Buttons
iButton_inputfiles = ttk.Button(iTk_main,text='Browse',command=f_inputfiles)
iButton_outputfile = ttk.Button(iTk_main,text='Browse',command=f_outputfile)
iButton_help = ttk.Button(iTk_main,text='Help',command=f_help)
iButton_run = ttk.Button(iTk_main,text='Run',command=f_run)
